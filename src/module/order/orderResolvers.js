...
export const orderProducts = authMiddleware(async (_, { products}, context) => {
    try {
      const {user} = context
...
export const getAllOrders = authMiddleware(async (_, {}, context) => {
  try {
    const { user } = context;
    const orders = await dynamoDB
      .query({
        ...
        ExpressionAttributeValues: marshall({
          ":orderedBy": user.id, // Passing the userId as a value
        }),
      })
      .promise();
...
export const getConsumer = authMiddleware(async (consumerId, {}, context) => {
  try {
...
export const getOrderedProducts = authMiddleware(async (items, {}, context) => {
  try {
